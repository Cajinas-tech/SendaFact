...
import { 
  ...
  Lock, Unlock, AlertCircle, CheckCircle2, 
  Printer, RefreshCw, PlusCircle, CreditCard, Banknote
} from 'lucide-react';
...
export const CashPage: React.FC = () => {
  ...
  const cardSales = todaySales.filter(s => s.payment_method === 'card').reduce((sum, s) => sum + s.total, 0);
  const transferSales = todaySales.filter(s => s.payment_method === 'transfer').reduce((sum, s) => sum + s.total, 0);
  const creditSales = todaySales.filter(s => s.payment_method === 'credit').reduce((sum, s) => sum + s.total, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/40 p-6 rounded-2xl border border-slate-800/80 backdrop-blur-md">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-amber-600 to-amber-400 p-0.5 shadow-lg shadow-amber-500/20 flex items-center justify-center">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Gestión de turnos, fondos de apertura, entradas/salidas y cuadre diario</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-card p-6 relative overflow-hidden group">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800">
            ...
            <span className="text-emerald-400 font-semibold">+{todaySales.length} ventas</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 relative overflow-hidden group">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800">
            ...
            <span className="text-slate-500">Transf: C$ {transferSales.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 relative overflow-hidden group">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-800">
            ...
            <span className="text-amber-400 font-bold">C$ {totalSales.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {registers.map((reg) => (
                <tr key={reg.id} className="hover:bg-slate-800/30 transition">
                  <td className="px-6 py-4 font-mono font-bold text-slate-200">{reg.id}</td>
                  <td className="px-6 py-4 font-semibold text-white">{reg.user}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">Apertura de Turno de Caja</h3>
                <p className="text-xs text-slate-400">Establece el monto de fondo inicial en efectivo</p>
              </div>
            </div>

            <form onSubmit={handleOpenRegister} className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5">Monto Inicial (C$)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl pl-10 pr-4 py-3 text-white font-mono font-bold text-lg focus:outline-none focus:border-emerald-500"
                    placeholder="1000.00"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5">Observaciones</label>
                <textarea
                  value={notes}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">Arqueo y Cierre de Caja</h3>
                <p className="text-xs text-slate-400">Turno: {activeRegister.id} • Cajero: {activeRegister.user}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800 space-y-2 text-sm">
              <div className="flex justify-between text-slate-400">
                ...
                <span className="text-white font-mono">C$ {activeRegister.initial_cash.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Ventas en Efectivo:</span>
                <span className="text-emerald-400 font-mono font-bold">+ C$ {cashSales.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Ventas Tarjeta / Transf:</span>
                <span className="text-blue-400 font-mono font-bold">C$ {(cardSales + transferSales).toFixed(2)}</span>
              </div>
              <div className="pt-2 border-t border-slate-800 flex justify-between font-bold text-base">
                <span className="text-white">Total Efectivo a Entregar:</span>
                <span className="text-amber-400 font-mono">C$ {activeRegister.current_cash.toFixed(2)}</span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleCashMovement} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5">Monto (C$)</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
                  ...
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white font-mono font-bold text-lg focus:outline-none focus:border-amber-500"
                  placeholder="0.00"
                />
              </div>

              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5">Motivo / Justificación</label>
                <input
                  type="text"
                  required
